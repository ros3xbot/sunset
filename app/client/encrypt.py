...
def decrypt_xdata(api_key: str, encrypted_payload: dict) -> dict:
    if not isinstance(encrypted_payload, dict) or "xdata" not in encrypted_payload or "xtime" not in encrypted_payload:
        raise ValueError("Invalid encrypted data format. Expected dict with 'xdata' and 'xtime'.")
    plaintext = dec_xdata(encrypted_payload["xdata"], int(encrypted_payload["xtime"]))
    return json.loads(plaintext)


def get_x_signature_payment(api_key: str, access_token: str, sig_time_sec: int,
                            package_code: str, token_payment: str,
                            payment_method: str, payment_for: str, path: str) -> str:
    return make_x_signature_payment(access_token, sig_time_sec, package_code,
                                    token_payment, payment_method, payment_for, path)


def get_x_signature_bounty(api_key: str, access_token: str, sig_time_sec: int,
                           package_code: str, token_payment: str) -> str:
    return make_x_signature_bounty(access_token, sig_time_sec, package_code, token_payment)


def get_x_signature_bounty_allotment(api_key: str, sig_time_sec: int, package_code: str,
                                     token_confirmation: str, destination_msisdn: str, path: str) -> str:
...
def get_x_signature_loyalty(api_key: str, sig_time_sec: int, package_code: str,
                            token_confirmation: str, path: str) -> str:
