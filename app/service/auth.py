...
class Auth:
        ...
        with open("refresh-tokens.json", "r", encoding="utf-8") as f:
            ...
            for rt in refresh_tokens:
                if "number" in rt and "refresh_token" in rt:
                    ...
                    print_warning("⚠️ Auth", f"Invalid token entry: {rt}")
    ...
    def add_refresh_token(self, number: int, refresh_token: str):
        ...
        if existing:
            ...
            if not tokens:
                print_error("❌ Auth", f"Failed to fetch new token for number {number}")
                return
    ...
    def remove_refresh_token(self, number: int):
        ...
        if self.active_user and self.active_user["number"] == number:
            if len(self.refresh_tokens) != 0:
                ...
                print_warning("⚠️ Auth", "No users left after removal.")
                self.active_user = None

    def set_active_user(self, number: int):
        rt_entry = next((rt for rt in self.refresh_tokens if rt["number"] == number), None)
        if not rt_entry:
            print_error("❌ Auth", f"No refresh token found for number: {number}")
            return False

        tokens = get_new_token(self.api_key, rt_entry["refresh_token"], rt_entry.get("subscriber_id", ""))
        if not tokens:
            print_error("❌ Auth", f"Failed to get tokens for number: {number}. Refresh token might be invalid or expired.")
            return False
        ...
        print_success("✅ Auth", f"Active user set to {number}")
        return True
    ...
    def renew_active_user_token(self):
        if self.active_user:
            ...
            if tokens:
                ...
                print_success("✅ Auth", "Active user token renewed successfully.")
                return True
            else:
                print_error("❌ Auth", "Failed to renew active user token.")
        else:
            print_error("❌ Auth", "No active user set or missing refresh token.")
        return False
    ...
    def edit_account_name(self, number: int, new_name: str):
        for user in self.refresh_tokens:
            if user["number"] == number:
                ...
                break
        self.write_tokens_to_file()
        print_success("✅ Auth", f"Account name for {number} updated to {new_name}")
